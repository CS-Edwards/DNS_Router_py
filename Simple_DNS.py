...
def getrecords(data):
    ...
    if qtype == b'\x00\x1C': #28 AAAA for IPv6
        qt='AAAA'
...
def rectobytes(domainname,rectype,recttl,recval):
    rbytes=b'\xc0\x0c' #python 2
    
    if rectype=='A':
        rbytes=rbytes+bytes([0])+bytes([1]) #python 3
    
    if rectype=='AAAA':
        rbytes=rbytes+bytes([0])+bytes([28]) #python 3
        
    rbytes=rbytes+bytes([0])+bytes([1])
    
    rbytes+=int(recttl).to_bytes(4,byteorder='big') #TTL 4 bytes RTC 1035
...
def buildq(domainname,rectype):
    ...
    for part in domainname:
        ...
        for char in part: 
            qbytes+=ord(char).to_bytes(2,byteorder='big') #ord() converts ASCII chars in to dec nums
...
def buildresponse(data):
    ...
    dnsQuestion= buildq(domainname,rectype)
